...
const INDUSTRIES_API_URL = 'https://smart-recommendation-dev.allvestfinance.in/api/analysis/getAvailableIndustries';
const USERNAME = 'allvest-admin';
const PASSWORD = 'dece5c8db75b0cd9';
...
async function fetchIndustries() {
  try {
    ...
    if (response.data && response.data.code === 2000 && Array.isArray(response.data.data)) {
      return response.data.data;
    } else if (response.data && response.data.code === 2004) {
      throw new Error("No industries available.");
    } else {
      throw new Error(response.data.msg || "Unexpected response structure from industries API.");
    }
  } catch (error) {
    console.error('Failed to fetch industries:', error);
    if (error.response) {
      throw new Error(`Error: ${error.response.status}. Unable to fetch industries.`);
    } else if (error.request) {
      throw new Error('Network Error: Could not connect to the industries API.');
    } else {
...
function getPeRatioClass(peRatio) {
  if (peRatio < 0 || !peRatio) return 'text-red-500';
...
function renderTablePaginationControls(totalRows) {
  ...
  pagination.className = 'pagination flex justify-end mt-4 space-x-2';
...
async function populateIndustries() {
  try {
    ...
    industrySelect.innerHTML = '<option value="" disabled selected>Failed to load industries</option>';
  }
}
...
toggleDetailedBtn.addEventListener('click', () => {
  ...
  toggleDetailedBtn.textContent = detailedSection.classList.contains('hidden') ? 'Show Detailed Analysis' : 'Hide Detailed Analysis';
});
...
function renderAllData() {
  ...
  if (!data) return;
  industryNameSpan.textContent = currentAnalysisData.industry_analyzed || industrySelect.value;
